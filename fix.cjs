...
const regex = /\) : subTab === 'cetak-slip' \? eventConfig\.isPrintOpen === false \? \([\s\S]*?<PrintRegistrationSlip[\s\S]*?\/>\n                    \) : eventConfig\.isEcertOpen === false \? \([\s\S]*?<PrintECert[\s\S]*?\/>\n                    \)\n                  }\n                  }/;

const fixed = `) : subTab === 'cetak-slip' ? eventConfig.isPrintOpen === false ? (
